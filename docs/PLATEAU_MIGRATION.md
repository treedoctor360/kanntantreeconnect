...
> **2026-08-23 実装済み**（`crownWidth` / `vegClass`）。
>
> **`girthHeight`（幹周の測定高）は入れないことにした**（Koh の判断・2026-08-23）。
> 理由:
>   - **幹周を根元で測ることはない。** 幹周と言えば胸高で測るものと決まっている
>   - 1.3m で測る地域もあるが、その差を気にするほど厳密な運用ではない
>   - したがって測定高を記録する意味がない
>
> このメモの当初案（4-2の表）では「後から遡れない」ため必須としていたが、
> **実務では測定高が一意に決まっているので、その懸念は当たらない。**
> 移行時に `veg:trunkDiameter` の意味が要るなら「胸高」と一律に扱えばよい。
> 再検討して復活させないこと。
...
| ~~`girthHeight`~~ | ~~幹周測定高~~ | — | **入れない**（上の注記参照。幹周は胸高で測ると決まっている） |

【推定】樹冠は「幅」1項目で足りると考えている。CityGMLの `crownDiameter` は水平投影の最大直径1値なので、
長径・短径を分けて取る必要はない。

~~**`girthHeight` が地味に重要**：胸高直径は測定高が分からないと比較できない。~~
→ **却下（2026-08-23）。** 幹周は胸高で測るものと決まっており、根元で測ることはない。
1.3m の地域差はあるがその程度の差を問題にする運用ではないので、記録しない。
